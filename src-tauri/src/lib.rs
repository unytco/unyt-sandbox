...
pub fn happ_bundle() -> AppBundle {
    info!("Loading happ bundle from workdir/unyt.happ");
    let bytes = include_bytes!("../../unyt/workdir/unyt.happ");
    info!("Happ bundle bytes loaded, size: {} bytes", bytes.len());
    let bundle = AppBundle::unpack(&bytes[..]).expect("Failed to decode unyt happ");
    info!("Happ bundle decoded successfully");
    bundle
}
...
pub fn run() {
    ...
    info!("Building Tauri application with plugins");
    ...
    info!("Added holochain plugin (manual launch mode)");
    ...
    builder = builder.setup(move |app| {
        ...
        #[cfg(mobile)]
        {
            info!("Mobile platform detected, adding mobile-specific plugins");
            app.handle().plugin(tauri_plugin_barcode_scanner::init())?;
            info!("Added barcode scanner plugin");
            app.handle()
                .plugin(tauri_plugin_safe_area_insets_css::init())?;
            info!("Added safe area insets CSS plugin");
        }
        #[cfg(not(mobile))]
        {
            info!("Desktop platform detected, adding desktop-specific plugins");
            // let h = app.handle();
            // app.handle()
            ...
            info!("Added updater plugin");
        }
        let handle = app.handle().clone();
        info!("Setting up holochain setup-completed event listener");
    ...
    #[cfg(not(mobile))]
    {
        info!("Adding desktop menu");
        builder = builder.menu(|handle| menu::build_menu(handle));
        info!("Desktop menu added");
    }

    info!("Starting Tauri application run loop");
    builder
        .run(tauri::generate_context!())
        .expect("error while running tauri application");
    info!("Tauri application has exited");
}

async fn open_window(handle: AppHandle) -> anyhow::Result<WebviewWindow> {
    info!("open_window: Creating main window");
    let app_config = app_config::AppConfig::new(&handle);
    debug!(
        "open_window: App config created with app_id: {}",
        app_config.app_id
    );
    ...
    let mut window_builder = handle
        ...
        .await?;
    info!("open_window: Window builder created");

    #[cfg(not(mobile))]
    {
        info!("open_window: Configuring desktop window properties");
        window_builder = window_builder
            ...
            .transparent(false);
        debug!(
            "open_window: Desktop window configured with title 'Unyt', maximized, and default size 1400x1000"
        );
    }

    info!("open_window: Building window");
    let window = window_builder.build()?;
    window.hide()?; // Ensure it stays hidden until explicitly shown by close_splashscreen
    info!("open_window: Window built successfully and hidden");
    Ok(window)
}
...
async fn setup(handle: AppHandle) -> anyhow::Result<()> {
    ...
    info!("setup: Starting application setup");
    let admin_ws = handle.holochain()?.admin_websocket().await?;
    state_manager.update_status(EnvRuntimeStatus::AppInstalling);
    info!("setup: Connected to admin websocket");

    let app_config = AppConfig::new(&handle);
    debug!(
        "setup: App config created with app_id: {}",
        app_config.app_id
    );
    ...
    if !app_is_already_installed {
        info!("setup: App not installed, checking for previous versions");
        ...
        if let Some(prev_app) = &previous_app {
            debug!(
                "setup: Found previous app version: {}",
                prev_app.installed_app_id
            );
        } else {
            info!("setup: No previous app versions found");
        }

        let mut roles_settings: RoleSettingsMap = RoleSettingsMap::new();
        debug!(
            "setup: Creating role settings for alliance with network_seed: {:?}",
        ...
        if let Some(previous_app) = previous_app {
            info!(
                "setup: Migrating from previous app: {}",
            ...
            info!("setup: Previous app disabled");
        } else {
            info!(
                "setup: Installing new app: {}",
                app_config.app_id
            );
            handle
                .holochain()?
