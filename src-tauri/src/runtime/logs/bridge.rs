...
pub fn init(app_handle: AppHandle) -> anyhow::Result<()> {
    ...
    // File Logs Output Layer — writes daily-rotated log files to app_log_dir
    let log_dir = app_handle
        .path()
        .app_log_dir()
        .expect("Could not resolve app log dir");
    std::fs::create_dir_all(&log_dir)
        .expect("Could not create app log dir");
    let file_appender = tracing_appender::rolling::daily(&log_dir, "unyt.log");
    ...
    // Leak the guard so the non-blocking writer lives for the lifetime of the process
    ...
    let _ = tracing_subscriber::registry()
        ...
        .with(file_layer) // Sink 3: Writes logs to file
        .try_init();
