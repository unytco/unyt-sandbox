...
- **Release pipeline: the macOS runner labels are pinned.** `macos-latest` has meant macOS 26 since mid-2026, whose screen-capture rules differ from the images phase 1 is proven on, so the two macOS build legs now run on `macos-15` — the same image family the smoke runs on (`macos-15` / `macos-15-intel`, from the inventory's matrix). The shipped macOS artifacts are therefore built against the macOS 15 SDK rather than whatever the rolling label points at that week, and `test-oracle.sh` fails the release if a `macos-latest` returns to either workflow.
